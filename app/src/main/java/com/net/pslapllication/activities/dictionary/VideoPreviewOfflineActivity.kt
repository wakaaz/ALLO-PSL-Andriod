...
import com.net.pslapllication.adpters.VideoPreviewOfflineAdapter
...
import kotlinx.android.synthetic.main.activity_video_preview_offline.btn_replay
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_bottom_options
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_centerscreen
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_dimview
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_download
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_favourite
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_share
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_toolbaar
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_vimeo
import kotlinx.android.synthetic.main.activity_video_preview_offline.constraint_youtube
import kotlinx.android.synthetic.main.activity_video_preview_offline.error_layout
import kotlinx.android.synthetic.main.activity_video_preview_offline.image_btn_menu
import kotlinx.android.synthetic.main.activity_video_preview_offline.img_btn_play_next
import kotlinx.android.synthetic.main.activity_video_preview_offline.img_btn_play_pause_center
import kotlinx.android.synthetic.main.activity_video_preview_offline.img_btn_pre
import kotlinx.android.synthetic.main.activity_video_preview_offline.mainlayout
import kotlinx.android.synthetic.main.activity_video_preview_offline.nestedScrollView
import kotlinx.android.synthetic.main.activity_video_preview_offline.progress
import kotlinx.android.synthetic.main.activity_video_preview_offline.recycler_next
import kotlinx.android.synthetic.main.activity_video_preview_offline.reload_layout
import kotlinx.android.synthetic.main.activity_video_preview_offline.switch_next
import kotlinx.android.synthetic.main.activity_video_preview_offline.tv_main
import kotlinx.android.synthetic.main.activity_video_preview_offline.tv_translate
import kotlinx.android.synthetic.main.activity_video_preview_offline.videoview
...
class VideoPreviewOfflineActivity : BaseActivity(), View.OnClickListener,
    ...
    //list
    public var list: List<DictionaryDataAPI>? = null
    ...
    private var adapter: VideoPreviewOfflineAdapter? = null
    var mediaPlayer: MediaPlayer? = null

    var context: Context = this
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_video_preview_offline)
    ...
    private fun setAdapter() {
        ...
        adapter = VideoPreviewOfflineAdapter(this, Constants.TYPE_VIDEO, "", this)
    ...
    private fun getIntentData() {
        if (intent != null &&
            ...
            selectedModel =
                intent.getSerializableExtra(Constants.SELECTED_DICTIONARY_LIST_MODEL) as DictionaryDataAPI
            ...
            Handler().postDelayed({
                list = ProgressHelper.getInstance(context)?.getListOffline()
                if (list?.size!=0){
            ...
            /* Handler().postDelayed({
                var DictionaryDataAPI: DictionaryDataAPI? = null
                DictionaryDataAPI?.id = selectedId
                if (list != null && list!!.size != 0 && selectedModel != null && (selectedModel!! as DictionaryDataAPI).id != null) {
                    val finalList = ListSorting.sortList(
                        (selectedModel!! as DictionaryDataAPI).indexPosition,
                        list!!
                    )
                    ...
                    if (isNetworkAvailable) {
                        if (progress.visibility == View.GONE) {
                            if (networkType == Constants.NetworkTYPE_MOB_DATA) {
                                if (ReuseFunctions.networkQualityMobData(this@VideoPreviewOfflineActivity) ==
                                    SharedPreferenceClass.getInstance(this@VideoPreviewOfflineActivity)
                                        ?.getQualityMob()
                                ) {
                                    //do nothing when quality doesn't change..
                                    ReuseFunctions.showToast(
                                        this@VideoPreviewOfflineActivity,
                                        "do nothing"
                                    )
                                } else {

                                    SharedPreferenceClass.getInstance(this@VideoPreviewOfflineActivity)
                                        ?.setQualityMob(ReuseFunctions.networkQualityMobData(this@VideoPreviewOfflineActivity))
                                    setUrlOfQuality(ReuseFunctions.networkQualityMobData(this@VideoPreviewOfflineActivity))

                                }
                            } else if (networkType == Constants.NetworkTYPE_WIFI) {
                                if (ReuseFunctions.networkQualityWifi(this@VideoPreviewOfflineActivity) ==
                                    SharedPreferenceClass.getInstance(this@VideoPreviewOfflineActivity)
                                        ?.getQualityWifi()
                                ) {
                                    //do nothing when quality doesn't change..
                                    ...
                                    if (SharedPreferenceClass.getInstance(this@VideoPreviewOfflineActivity)
                                            ?.getPos() != ""
                                    ) {
                                        SharedPreferenceClass.getInstance(this@VideoPreviewOfflineActivity)
                                            ?.setQualityWifi(ReuseFunctions.networkQualityWifi(this@VideoPreviewOfflineActivity))
                                        setUrlOfQuality(ReuseFunctions.networkQualityWifi(this@VideoPreviewOfflineActivity))
                                    }
                                }
                            }
                        }
    ...
    private fun setUrlOfQuality(isStatusChabge: String) {
        when (isStatusChabge) {
            "POOR" -> {
                if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p240p?.urlp240 != null) {
                    var videoUrl: String =
                        URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p240p.urlp240)
            ...
            "MODERATE" -> {
                if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p360p?.urlp360p != null) {
                    var videoUrl: String =
                        URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p360p.urlp360p)
                    setUrlBasedOnQuality(videoUrl)
                }
            ...
            "GOOD" -> {
                if (selectedModel != null && (selectedModel as DictionaryDataAPI?)?.p480p?.urlp480 != null) {
                    var videoUrl: String =
                        URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p480p.urlp480)
                    setUrlBasedOnQuality(videoUrl)
                }
            ...
            "EXCELLENT" -> {
                if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p720p?.urlp720 != null) {
                    var videoUrl: String =
    ...
    private fun setUrlBasedOnQuality(url: String) {
        runOnUiThread {
            if (SharedPreferenceClass.getInstance(this@VideoPreviewOfflineActivity)?.getPos() == "") {
                //do nothing
            } else {
                setplayer(url)
                val roundedBalance =
                    SharedPreferenceClass.getInstance(this@VideoPreviewOfflineActivity)?.getPos()
    ...
    private fun setTitleText(selectedModel: DictionaryDataAPI) {
        ...
        when (categoryType) {
            Constants.TYPE_DICTIONARY -> {
                removeFromFavouriteCall(
                    (selectedModel as DictionaryDataAPI?)!!.id.toString(),
                    "0",
                    "0",
                    "0",
                    "0"
    ...
    private fun shareSheet() {
        ...
        val shareBody = URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p720p!!.urlp720)
    ...
    private fun openDownloadBottomSheet() {
        if (selectedModel != null) {
            ...
            try {
                Glide.with(context)
                    .load(URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.poster))
            ...
            dialogView.tv_low.text = (selectedModel as DictionaryDataAPI?)!!.p240p.filesizep240
            dialogView.radiogroup.setOnCheckedChangeListener { arg0, arg1 ->
                ...
                try {
                    if (selectedId == R.id.radio_btn_high) {
                        ...
                        if ((selectedModel as DictionaryDataAPI?)?.p720p?.urlp720 != null)
                            selected_url = (selectedModel as DictionaryDataAPI?)!!.p720p!!.urlp720
                    }
                    if (selectedId == R.id.radio_btn_medium) {
                        if ((selectedModel as DictionaryDataAPI?)?.p480p?.urlp480 != null)
                            selected_url =
                                (selectedModel as DictionaryDataAPI?)!!.p480p.urlp480
                    }
                    if (selectedId == R.id.radio_btn_low) {
                        if ((selectedModel as DictionaryDataAPI?)?.p240p?.urlp240 != null)
                            selected_url =
                                (selectedModel as DictionaryDataAPI?)!!.p240p.urlp240
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
    ...
    override fun onResume() {
        ...
        if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p720p?.urlp720 != null) {
            val videoUrl: String = URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p720p!!.urlp720)
            setplayer(videoUrl)
            videoview.start()
        }
        videoview.visibility = View.VISIBLE
    ...
    private fun checkPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (selectedId == -1) {
                ...
                if ((selectedModel as DictionaryDataAPI?) != null)
                    try {
                        newDownload(
                            URLDecoder.decode(selected_url),
                            (selectedModel as DictionaryDataAPI?)!!.filename,
                            (selectedModel as DictionaryDataAPI?)!!.poster
                        )
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
    ...
    override fun onQualityChangeSelected(qualityId: String) {
        ...
        when (qualityId) {
            "0" -> {
                if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p240p?.urlp240 != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p240p.urlp240))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
            ...
            "1" -> {
                if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p360p?.urlp360p != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p360p.urlp360p))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
            ...
            "2" -> {
                if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p480p?.urlp480 != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p480p.urlp480))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
            ...
            "3" -> {
                if ((selectedModel as DictionaryDataAPI?) != null && (selectedModel as DictionaryDataAPI?)?.p720p?.urlp720 != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p720p!!.urlp720))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
                ...
                override fun onResponse(
                    ...
                    if (addToFvouriteModel != null && addToFvouriteModel.code == 200) {
                        if (addToFvouriteModel.data != null) {
                            if (!this@VideoPreviewOfflineActivity.isDestroyed) {
    ...
    override fun onVideoSelect(selectedModelVideo: DictionaryDataAPI) {
        selectedModel = selectedModelVideo
        if (selectedModel != null && (selectedModel as DictionaryDataAPI?)?.p720p?.urlp720 != null) {
            try {
                var DictionaryDataAPITemp: DictionaryDataAPI? =  null
                DictionaryDataAPITemp= selectedModel as DictionaryDataAPI
                var newIndex = list!!.indexOf(DictionaryDataAPITemp)
                val videoUrl: String =
                    URLDecoder.decode((selectedModel as DictionaryDataAPI?)!!.p720p!!.urlp720)
                setNormalVideoViews()
                setplayer(videoUrl)
            ...
            try {
                ...
                if (list?.size != null) {
                    ...
                    try {
                        ...
                        if (list != null) {
                            val finalList =
                                ListSorting.sortListWordsOffline(
                                    newIndex,
                                    list!!
                                )
                            setNextVideosList(finalList)
    ...
    public class DownloadProgressCounter(
        ...
        var context: Context = VideoPreviewOfflineActivity()
        override fun run() {
            while (downloadId > 0) {
                try {
