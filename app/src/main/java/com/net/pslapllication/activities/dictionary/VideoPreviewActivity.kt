...
class VideoPreviewActivity : BaseActivity(), View.OnClickListener,
    ...
    //list
    public var list: List<DictionaryData>? = null
    ...
    private var adapter: VideoPreviewAdapter? = null
    var mediaPlayer: MediaPlayer? = null
    var context: Context = this@VideoPreviewActivity
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_video_preview)
        SharedPreferenceClass.getInstance(this@VideoPreviewActivity)
            ?.setPos("0.0")
        getIntentUrl()
    ...
    private fun setAdapter() {
        ...
        adapter = VideoPreviewAdapter(this, Constants.TYPE_VIDEO, "", this)
    ...
    private fun getIntentData() {
        if (intent != null &&
            ...
            selectedModel =
                intent.getSerializableExtra(Constants.SELECTED_DICTIONARY_LIST_MODEL) as DictionaryData
            if (selectedModel != null) {
                setTitleText((selectedModel as DictionaryData))
            }

            /********get complete list data*********/

            Handler().postDelayed({
                list = ProgressHelper.getInstance(context)?.getList()
                if (list?.size != 0) {
            ...
            /* Handler().postDelayed({
                ...
                dictionaryData?.id = selectedId
                if (list != null && list!!.size != 0 && selectedModel != null && (selectedModel!! as DictionaryData).id != null) {
                    val finalList = ListSorting.sortList(
                        (selectedModel!! as DictionaryData).indexPosition,
                        list!!
                    )
    ...
    class LongOperation(
        var list: List<DictionaryData>?, var selectedId: Int, var selectedModel: DictionaryData?,
        var adapter: VideoPreviewAdapter?, var nestedScrollView: CustomScrollView
    ) :
        AsyncTask<String?, Void?, String?>() {
        override fun doInBackground(vararg params: String?): String {
            //list = Constants.carrierData?.getModelList()
        ...
        override fun onPostExecute(result: String?) {
            var dictionaryData: DictionaryData? = null
            dictionaryData?.id = selectedId
            if (list != null && selectedModel != null && (selectedModel as DictionaryData)?.id != null) {
                val finalList =
                    ListSorting.sortList((selectedModel as DictionaryData)?.indexPosition!!, list!!)
                if (adapter != null) {
                    adapter!!.setWords(finalList!!)
                    ...
                    if (isNetworkAvailable) {
                        if (progress.visibility == View.GONE) {
                            if (networkType == Constants.NetworkTYPE_MOB_DATA) {
                                if (ReuseFunctions.networkQualityMobData(this@VideoPreviewActivity) ==
                                    SharedPreferenceClass.getInstance(this@VideoPreviewActivity)
                                        ?.getQualityMob()
                                ) {
                                    ...
                                    SharedPreferenceClass.getInstance(this@VideoPreviewActivity)
                                        ?.setQualityMob(ReuseFunctions.networkQualityMobData(this@VideoPreviewActivity))
                                    setUrlOfQuality(ReuseFunctions.networkQualityMobData(this@VideoPreviewActivity))

                                }
                            } else if (networkType == Constants.NetworkTYPE_WIFI) {
                                if (ReuseFunctions.networkQualityWifi(this@VideoPreviewActivity) ==
                                    SharedPreferenceClass.getInstance(this@VideoPreviewActivity)
                                        ?.getQualityWifi()
                                ) {
                                    //do nothing when quality doesn't change..
                                    ...
                                    if (SharedPreferenceClass.getInstance(this@VideoPreviewActivity)
                                            ?.getPos() != ""
                                    ) {
                                        SharedPreferenceClass.getInstance(this@VideoPreviewActivity)
                                            ?.setQualityWifi(ReuseFunctions.networkQualityWifi(this@VideoPreviewActivity))
                                        setUrlOfQuality(ReuseFunctions.networkQualityWifi(this@VideoPreviewActivity))
                                    }
                                }
                            }
                        }
    ...
    private fun setUrlOfQuality(isStatusChabge: String) {
        when (isStatusChabge) {
            "POOR" -> {
                if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p240p?.url != null) {
                    var videoUrl: String =
                        URLDecoder.decode((selectedModel as DictionaryData?)!!.p240p.url)
            ...
            "MODERATE" -> {
                if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p360p?.url != null) {
                    var videoUrl: String =
                        URLDecoder.decode((selectedModel as DictionaryData?)!!.p360p.url)
                    setUrlBasedOnQuality(videoUrl)
                }
            ...
            "GOOD" -> {
                if (selectedModel != null && (selectedModel as DictionaryData?)?.p480p?.url != null) {
                    var videoUrl: String =
                        URLDecoder.decode((selectedModel as DictionaryData?)!!.p480p.url)
                    setUrlBasedOnQuality(videoUrl)
                }
            ...
            "EXCELLENT" -> {
                if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p720p?.url != null) {
                    var videoUrl: String =
                        URLDecoder.decode((selectedModel as DictionaryData?)!!.p720p.url)
                    setUrlBasedOnQuality(videoUrl)
                }
    ...
    private fun setUrlBasedOnQuality(url: String) {
        runOnUiThread {
            if (SharedPreferenceClass.getInstance(this@VideoPreviewActivity)?.getPos() == "") {
                //do nothing
            } else {
                setplayer(url)
                val roundedBalance =
                    SharedPreferenceClass.getInstance(this@VideoPreviewActivity)?.getPos()
    ...
    private fun setAlreadyFavourite() {
        if (selectedModel != null) {
            if ((selectedModel as DictionaryData?)!!.favorite != null && (selectedModel as DictionaryData?)!!.favorite == 0) {
    ...
    private fun shareSheet() {
        ...
        val shareBody = URLDecoder.decode((selectedModel as DictionaryData?)!!.shareablURL)
    ...
    private fun openDownloadBottomSheet() {
        if (selectedModel != null) {
            ...
            try {
                Glide.with(context)
                    .load(URLDecoder.decode((selectedModel as DictionaryData?)!!.poster))
            ...
            dialogView.tv_main.text = (selectedModel as DictionaryData?)!!.english_word
            ...
            dialogView.tv_low.text = (selectedModel as DictionaryData?)!!.p240p.filesize
            dialogView.radiogroup.setOnCheckedChangeListener { arg0, arg1 ->
                ...
                try {
                    if (selectedId == R.id.radio_btn_high) {
                        ...
                        if ((selectedModel as DictionaryData?)?.p720p?.url != null)
                            selected_url = (selectedModel as DictionaryData?)!!.p720p.url
                    }
                    if (selectedId == R.id.radio_btn_medium) {
                        if ((selectedModel as DictionaryData?)?.p480p?.url != null)
                            selected_url =
                                (selectedModel as DictionaryData?)!!.p480p.url
                    }
                    if (selectedId == R.id.radio_btn_low) {
                        if ((selectedModel as DictionaryData?)?.p240p?.url != null)
                            selected_url =
                                (selectedModel as DictionaryData?)!!.p240p.url
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
    ...
    override fun onResume() {
        ...
        if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p720p?.url != null) {
            val videoUrl: String = URLDecoder.decode((selectedModel as DictionaryData?)!!.p720p.url)
            setplayer(videoUrl)
            videoview.start()
        }
        videoview.visibility = View.VISIBLE
    ...
    override fun onQualityChangeSelected(qualityId: String) {
        ...
        when (qualityId) {
            "0" -> {
                if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p240p?.url != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryData?)!!.p240p.url))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
            ...
            "1" -> {
                if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p360p?.url != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryData?)!!.p360p.url))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
            ...
            "2" -> {
                if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p480p?.url != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryData?)!!.p480p.url))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
            ...
            "3" -> {
                if ((selectedModel as DictionaryData?) != null && (selectedModel as DictionaryData?)?.p720p?.url != null) {
                    try {
                        setVideoQuality(URLDecoder.decode((selectedModel as DictionaryData?)!!.p720p.url))
                    } catch (e: UnsupportedEncodingException) {
                        e.printStackTrace()
                    }
                }
                ...
                override fun onResponse(
                    ...
                    if (addToFvouriteModel != null && addToFvouriteModel.code == 200) {
                        if (addToFvouriteModel.data != null) {
                            if (!this@VideoPreviewActivity.isDestroyed) {
    ...
    override fun onVideoSelect(selectedModelVideo: DictionaryData) {
        ...
        if (selectedModel != null && (selectedModel as DictionaryData?)?.p720p?.url != null) {
            try {
    ...
    private fun setNextPreVideo(value: Int) {
        if (selectedModel != null && (selectedModel as DictionaryData?)?.indexPosition != null) {
            try {
                var nextVideo = (selectedModel as DictionaryData?)?.indexPosition
    ...
    public class DownloadProgressCounter(
        ...
        var context: Context = VideoPreviewActivity()
        override fun run() {
            while (downloadId > 0) {
                try {
                    sleep(300)
